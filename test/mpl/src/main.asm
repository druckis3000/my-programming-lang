...
main_Masina_Ptr_printInfo_S:
	...
	; Push "rDydis: %d, greitis: %d, bako talpa: %d\n" onto the stack
	mov DWORD [esp + 0], lc0
	
	; Push the contents of variable 'this.ratuDydis' onto the stack
	mov ebx, [ebp + 8]
	mov ebx, DWORD [ebx + 0 + 0]
	mov DWORD [esp + 4], ebx
	
	; Push the contents of variable 'this.greitis' onto the stack
	mov ebx, [ebp + 8]
	mov ebx, DWORD [ebx + 4 + 0]
	mov DWORD [esp + 8], ebx
	
	; Push the contents of variable 'this.kuroBakas.talpa' onto the stack
	mov ebx, [ebp + 8]
	lea ebx, [ebx + 8]
	mov ebx, DWORD [ebx + 0 + 0]
...
main:
	...
	; Push the address of variable '&m' onto the stack
	lea ebx, [ebp - 60]
	mov DWORD [esp + 4], ebx
	...
	; Push the address of variable '&m.greitis' onto the stack
	lea ebx, [ebp - 60]
	lea ebx, [ebx + 4 + 0]
	mov DWORD [esp + 4], ebx
	...
	; printInfo(&m)
	call main_Masina_Ptr_printInfo_S
	...
	; setInfo(&m, 15, 113, 70)
	call main_Masina_Ptr_setInfo_Sddd
	...
	; Push "*mPtr: %d, *(mPtr+8): %d\n" onto the stack
	mov DWORD [esp + 0], lc8
	
	; Push the contents of memory address, contained in variable '*mPtr' onto the stack
	...
	; Push the address of variable '&ms[0]' onto the stack
	lea ebx, [ebp + 0 - 44]
	mov DWORD [esp + 0], ebx
	...
	; setInfo(&ms[0], 11, 111, 1111)
	call main_Masina_Ptr_setInfo_Sddd

	; Push the address of variable '&ms[1]' onto the stack
	lea ebx, [ebp + 12 - 44]
	mov DWORD [esp + 0], ebx
	...
	.while0:
		...
		; Push the address of variable '&ms[i]' onto the stack
		mov ebx, DWORD [ebp - 20]
		imul ebx, 12
		lea ebx, [ebp + ebx - 44]
		mov DWORD [esp + 8], ebx
	...
	cmp eax, ebx
	jle .for0_end
		; Push the address of variable '&ms[z]' onto the stack
		mov ebx, DWORD [ebp - 16]
		imul ebx, 12
		lea ebx, [ebp + ebx - 44]
		mov DWORD [esp + 0], ebx
	...
	; {
		; Push the address of variable '&bibis' onto the stack
		lea ecx, [ebp - 12]
		mov DWORD [esp + 0], ecx
...
SECTION .rodata
lc8: db "*mPtr: %d, *(mPtr+8): %d", 0x0A, 0
lc7: db "", 0x0A, 0
lc9: db "&ms[%d]: %x", 0x0A, 0
lc0: db "rDydis: %d, greitis: %d, bako talpa: %d", 0x0A, 0
lc2: db "test", 0x0A, 0
lc1: db "c: %d, b: %d", 0x0A, 0
lc4: db "ayay", 0x0A, 0
lc3: db "no test!", 0x0A, 0
lc6: db "&m.greitis: %x", 0x0A, 0
lc5: db "&m: %x", 0x0A, 0